...
//    @Bean
//    public CommandLineRunner initTelegrammApi(ApplicationContext ctx) {
//        return args -> {
...
//    }
    ...
    CommandLineRunner loadDatabase(MoodRepository moodRepository,
        ...
        return args -> {
            initializeMoods(moodRepository, moodContentRepository);
            initializeAwards(awardRepository);
        };
    }

    private void initializeMoods(MoodRepository moodRepository,
                                 MoodContentRepository moodContentRepository) {
        var moods = moodRepository.findAll();
        if (!moods.isEmpty()) {
            return;
        }
        var data = createMoodContents();
    ...
    private void initializeAwards(AwardRepository awardRepository) {
    ...
    private List<Award> createAwards() {
        ...
        awards.add(new Award("Физический подарок", "После 60 дней хорошего настроения. "
                + "Награда: Возможность получить небольшой физический подарок, "
